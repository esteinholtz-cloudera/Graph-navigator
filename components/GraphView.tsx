...
const GraphView: React.FC<Props> = ({ data, physics, edgeConfig, grouping, selectedNodeId, onNodeSelect, onEdgeSelect, globalDegrees }) => {
  ...
  useEffect(() => {
    ...
    // Synchronize nodes with previous simulation state
    ...
    nodeMerged.select('.node-circle')
      ...
      .attr('fill', d => grouping.byType ? colorScale(d.community || d.type) : '#3b82f6')
      .attr('stroke', '#fff')
  ...
  }, [data, physics, edgeConfig, grouping, globalDegrees, selectedNodeId]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-slate-900 overflow-hidden relative" onClick={() => onNodeSelect(null)}>
      ...
      <div className="absolute bottom-6 right-6 flex flex-col gap-2">
        <button onClick={(e) => { e.stopPropagation(); handleZoomToFit(); }} className="p-2 bg-slate-800/80 hover:bg-blue-600 text-slate-300 hover:text-white rounded-lg border border-slate-700 shadow-xl backdrop-blur-md transition-all" title="Zoom to Fit">
